...
// 添加新的辅助函数用于生成更真实的时间戳
export const generateReceiptTime = (category: string) => {
  const now = new Date();
...
// 添加新的辅助函数用于生成更真实的支付信息
export const generatePaymentInfo = (amount: number, category: string) => {
...
// 添加商品描述生成器
export const generateItemDescription = (item: { name: string, price: number }) => {
  const brands = {
  ...
  const brand = brands[item.price > 20 ? 'luxury' : 'standard'][
...
// 添加商品分类功能
export const categorizeItems = (items: Array<any>) => {
  const categories = {
...
// 添加新的辅助函数
export const generateDepartmentCode = () => {
...
// 添加餐饮相关的描述生成器
export const generateFoodDescription = (item: { name: string, price: number }) => {
...
// 添加餐饮特定的支付信息生成
export const generateRestaurantPaymentInfo = (amount: number, category: string) => {
...
// 添加餐饮特定的订单信息生成
export const generateOrderInfo = (category: string) => {
  const baseInfo = {
    orderNumber: generateOrderNumber(),
